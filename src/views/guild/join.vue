<template>
  <div class="app-container">
    <cus-wraper>
      <cus-filter-wraper>
        <el-button v-has="'sys:user:add'" type="primary" @click="handleCreate" icon="el-icon-plus">{{ 'table.add' }}</el-button>
      </cus-filter-wraper>
      <div class="table-container">
        <el-table v-loading="listLoading" :data="list" size="mini" fit element-loading-text="Loading"
          ...
          <el-table-column  align="right" :label="'table.actions'">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" v-dialogDrag @close="handleDialogClose">
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible = false"> {{ 'table.cancel' }} </el-button>
          <el-button type="primary" @click="submitForm"> {{ 'table.confirm' }} </el-button>
        </span>
      </el-dialog>
    </cus-wraper>
  </div>
